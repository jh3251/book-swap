...
const ResetPasswordPage: React.FC = () => {
  ...
  useEffect(() => {
    ...
    if (!code) {
      setError(lang === 'bn' ? 'অকার্যকর লিংক।' : 'Invalid or expired link.');
    } else {
      setOobCode(code);
    }
  }, [location, lang]);
      ...
      <div className="bg-white p-8 md:p-12 rounded-[2.5rem] shadow-2xl shadow-emerald-900/10 border border-emerald-50">
        <div className="text-center mb-10">
          <div className="inline-flex items-center gap-2 px-4 py-1.5 bg-emerald-50 rounded-full border border-emerald-100 text-accent font-black text-[9px] uppercase mb-6">
            <Sparkles className="w-3 h-3" /> Security
          </div>
          <h1 className="text-4xl font-serif font-black text-black mb-3">
            {lang === 'bn' ? 'নতুন পাসওয়ার্ড সেট করুন' : 'Set New Password'}
          </h1>
          <p className="text-zinc-500 font-medium text-sm">
            {lang === 'bn' ? 'আপনার অ্যাকাউন্টের জন্য একটি শক্তিশালী পাসওয়ার্ড তৈরি করুন।' : 'Create a strong password for your account access.'}
          </p>
        </div>
        ...
        <form onSubmit={handleSubmit} className="space-y-6">
          ...
          <div>
            <label className="block text-[10px] font-black text-zinc-900 uppercase mb-3 ml-1 tracking-[0.1em]">
              New Password
            </label>
          ...
          >
            ...
            {lang === 'bn' ? 'পাসওয়ার্ড রিসেট করুন' : 'Update Password'}
          </button>
        </form>
      </div>
    </div>
