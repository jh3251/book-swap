        ...
        <div className="bg-white rounded-[2rem] md:rounded-[2.5rem] p-6 md:p-8 shadow-2xl shadow-emerald-900/5 border border-emerald-50">
          ...
          <nav className="flex lg:flex-col overflow-x-auto lg:overflow-visible gap-2 md:gap-3 no-scrollbar pb-2 lg:pb-0">
            <button 
              ...
              className={`flex-shrink-0 lg:w-full flex items-center justify-start gap-4 p-4 font-black rounded-2xl transition text-[10px] md:text-xs uppercase ${activeTab === 'ads' ? 'bg-accent text-white shadow-lg shadow-accent/20' : 'bg-emerald-50/50 text-black hover:bg-emerald-100'}`}
            >
              <LayoutGrid className="w-4 h-4 md:w-5 md:h-5" /> Ads
            </button>
            <button 
              onClick={() => setActiveTab('saved')}
              className={`flex-shrink-0 lg:w-full flex items-center justify-start gap-4 p-4 font-black rounded-2xl transition text-[10px] md:text-xs uppercase ${activeTab === 'saved' ? 'bg-accent text-white shadow-lg shadow-accent/20' : 'bg-emerald-50/50 text-black hover:bg-emerald-100'}`}
            >
      ...
      <main className="lg:col-span-3 space-y-6 md:space-y-8">
        {activeTab === 'ads' && (
          <>
            ...
            <div className="bg-orange-50/60 border border-orange-100/50 rounded-2xl px-6 py-4 flex items-center gap-4">
               <Info className="w-5 h-5 text-orange-400 flex-shrink-0" />
               <p className={`text-[11px] md:text-[13px] font-bold text-orange-800 leading-relaxed ${lang === 'bn' ? 'font-bn' : ''}`}>{t('deleteReminder')}</p>
            </div>
